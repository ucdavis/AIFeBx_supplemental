...
## Objective
Merge model output from lymphocyte detection model and mucosal compartment segmentation model
...
Each chunk is written to only run if the (key) output file is absent.
The chunks "outline", "fragments" and "lcMucosa" are extremely slow.
To bypass chunks, download the output files for the chunk (listed above each chunk) from bioImageArchive to the right location.

### Input
- Mucosa coords from aiforia (geojson in um) </input/mucosa_coords_aiforia/> 
- Lymphocyte coords from aiforia (geojson in um) </input/lymphocyte_coords_aiforia/> 

### Output (used in other scripts)
- Slide names - vector </output/mergeModels/all/slideNames.rds>
- Valid mucosa coordinates (mucosa coordinates updated with fragment info) </output/mergeModels/by_slide/validMucosa/coordinates/>
- Lymphocyte mucosa coordinates <output/mergeModels/by_slide/lcMucosa/coordinates/> 
- Lymphocyte nuclear segmentation coordinates <output/mergeModels/by_slide/lcSeg/coordinates/> 
- Density summary statistics per slide <output/mergeModels/all/slideSummary.rds>

### Intermediate output
- Raw mucosa coordinates <output/mergeModels/by_slide/rawMucosa/coordinates/> 
- Raw mucosa plots </output/mergeModels/by_slide/rawMucosa/plots/>
- Filtered mucosa coords </output/mergeModels/by_slide/filteredMucosa/coordinates/>
- Filtered mucosa plots </output/mergeModels/by_slide/filteredMucosa/plots/>
- Outline coordinates </output/mergeModels/by_slide/outlines/coordinates/>
- Outline plots <output/mergeModels/by_slide/outlines/plots/>
- Fragment coordinates (coordinates for the fragment polygons) </output/mergeModels/by_slide/fragments/coordinates/>
- Fragment metadata </output/mergeModels/by_slide/fragments/metadata/>
- Fragment order </output/mergeModels/by_slide/fragments/order/>
- Raw lymphocyte coordinates <output/mergeModels/by_slide/rawLc/coordinates/> 
- Raw lymphocyte plots </output/mergeModels/by_slide/rawLc/plots/>
- Lymphocyte point coordinates  <output/mergeModels/by_slide/lcPoint/coordinates/> 
- Fragment density metadata <output/mergeModels/by_slide/fragmentDensity/metadata/> 
- Fragment density coordinates (same as lymphocyte mucosa coordinates with added metadata) <output/mergeModels/by_slide/fragmentDensity/coordinates/> 
- Fragment density plots <output/mergeModels/by_slide/fragmentDensity/plots/> 
- Fragment density metadata for all slides <output/mergeModels/all/slideDensity.rds>
...
for (filePath in filePathsMucosa) {
  ...
  nameSlide <- nameParts[6] #use the 5th element as the slide name
...
for (name in slideNames){
  ...
  if (!any(sapply(list.files(rawMucosaCoords_dir), function(file_name) grepl(name, file_name)))) {
    ...
    for (filePath in filePathsMucosa) {
      ...
      nameSlide_mucosaAiforia <- nameParts_mucosaAiforia[6]
...
###Outlines (very slow)
Merges lamina propria and epithelial polygons to get tissue outlines
####Input
- Slide names - vector </output/mergeModels/all/slideNames.rds>
- Filtered mucosa coords </output/mergeModels/by_slide/filteredMucosa/coordinates/>
####Output
...
- Valid mucosa coordinates (mucosa coordinates updated with fragment info) </output/mergeModels/by_slide/validMucosa/coordinates/>
#### Bypass
- To bypass chunk (recommended), download validMucosa coordinates from bioimage archive (also fragmentCoords, fragmentOrder, fragment metaData) from bioimage archive (the chunk won't run if you have the fragment coordinates in place, but unless you also download the fragment metadata, fragment order and valid mucosa coordinates, these won't be generated and the rest of the script won't run)
```{r fragments}
...
for (name in slideNames){
  ...
  if (!any(sapply(list.files(validMucosaCoords_dir), function(file_name) grepl(name, file_name)))) {
...
### LcPoints
Transform lymphocyte nuclei polygons to lymphocyte centroid point coordinates ()
...
### LcMucosa (very slow)
Merge lymphocyte (centroid) coordinates with mucosal coordinates
####Input
- Slide names - vector </output/mergeModels/all/slideNames.rds>
- Valid mucosa coordinates (mucosa coordinates updated with fragment info) </output/mergeModels/by_slide/validMucosa/coordinates/>
- Lymphocyte point coordinates  <output/mergeModels/by_slide/lcPoint/coordinates/> 
...
### Summary stats
Calculate summary statistics for lymphocyte densities per fragment all slides
#### Input
- Slide names - vector </output/mergeModels/all/slideNames.rds>
- Fragment density metadata (per slide) <output/mergeModels/by_slide/fragmentDensity/metadata/> 
#### Output
- Fragment density metadata for all slides <output/mergeModels/all/slideDensity.rds>
- Density summary statistics per slide <output/mergeModels/all/slideSummary.rds>
```{r}
...
for (name in slideNames){
  ...
  if (length(matching_files_densityMetadata) > 0) {
    ...
    saveRDS(slideDensityMetadata, paste0(slideDensityMetadata_dir,"slideDensity.rds"))
    ...
    saveRDS(slideSummary, paste0(slideSummaryMetadata_dir,"slideSummary.rds"))
  }
}
```
