...
- The candidate lymphocyte plots are very large in tiff format (for paper). Changed to png, but saved the ones used for paper figures under figures/supplementary and figures/main. To reproduce tiff files, delete figures to reproduce from output, block code for generating pngs and unblock code for generating tiffs
...
annotationsSfCoords_dir <- paste0(outputDir_bySlide,"parsedAnnotations/coordinates/")
...
annotationsDistCoords_dir <-paste0(outputDir_bySlide,"distanceBetween/") 
...
#Prints are flipped vertical compared to original. This is due to image coordinates 0,0 being top left and and graphs starting bottom left. Can be fixed with a flipYcoord function, either in graph (alternative 1) or by changing the geometry (alternative 2) by multiply y coordinates with -1:
...
issueLabel <- c( "off_target_annotation" = "Colocalization (technical)", "clustering" = "Reference aggregation (technical)", "roi_border" = "Validation region border (technical)", "mucosal_model" = "Mucosal compartment model", "lymphocyte_model" = "Cell detection model" )
...
for (name in slideNames){
  ...
  matching_files_rawAnnotations <- NULL
...
# read in lcMucosa and lcSeg and define intersection with rois
...
- Plots of candidate lymphocytes (bounding boxes), and overlaid individual pathologist annotations (centers) and AI mucosa and lymphocyte outline predictions by slide <output/lymphocyteValidation/bySlide/candidateLc/<threshold>/plots"

#### Deviation
Did not set seed first time I ran this. Dbscan have random element. Got the same results rerunning though.
  ...
  for (name in slideNames){
    ...
    if(!any(sapply(list.files(subfolder_plots), function(file_name) grepl(name, file_name)))){
      ...
      if(length(matching_files_validMucosaRoi) > 0 && length(matching_files_lcSegRoi) > 0 && length(matching_files_buffers) > 0 && length(matching_files_sfAnnotations) > 0) {
        ...
        # Save the plot as a tif file - format for paper takes up too much space to run for all
        #tiff_file_path <- file.path(subfolder_plots, paste0(name,"_plot_th_", threshold_name, "_",currentDate, ".tif"))
        #tiff(tiff_file_path, width= 30, height = 14, units = "in", res=300) #resize to width 5 after
        #print(combined_plot)
        #dev.off()
        
        png_file_path <- file.path(subfolder_plots, paste0(name, "_plot_th_", threshold_name, "_", currentDate, ".png"))
        png(png_file_path, width = 30, height = 14, units = "in", res = 300) # resize to width 5 after
        print(combined_plot)
        dev.off()
      }
    }
...
tiff(output_file_tif, width = 7, height = 1.75, units = "in", res = 300)
print(stack_bar_candidate)
dev.off()
...
write_xlsx(reference_of_candidate_table, paste0(tables_dir, "reference_of_candidate_table.xlsx"))
```
...
#### Output
- Stats table for "Lymphocytes by source" (used in **Table 1**) as excel  <tables/lymphocytes_bySource.xlsx>
...
- Combined summaries tables as rds <output/lymphocyteValidation/all/combined_summaries.rds> (used for ranking)
...
#Save as rds
saveRDS(lymphocytes_bySource, paste0(outputDir_all,"lymphocytes_bySource.rds"))
#and excel
write_xlsx(lymphocytes_bySource, paste0(tables_dir, "lymphocytes_bySource.xlsx"))


### Histograms for lymphocytes per ROI 
candidateLc_summaries <- candidateLc_summaries %>%
  mutate(pathologist = "Cand")

referenceLc_summaries <- referenceLc_summaries %>%
  mutate(pathologist = "Ref")

predictionAI_summaries <- predictionAI_summaries %>%
...
plot <- ggplot(unequivocallyFalse, aes(x = "", fill = issue)) +
  ...
  labs(x = "Unequivocal FP & FN", y = "Percentage") +  # Change y-axis label to "Percentage"
...
plot <- ggplot(ueF_lc, aes(x = "", fill = factor(confusion_term))) +
  ...
  labs(x = "Unequivocally false \nwith cell detection error", y = "Percentage") +
...
plot <- ggplot(ueFP_lc, aes(x = "", fill = factor(object))) +
  geom_bar(position = "fill") +
  labs(x = "Unequivocal FP\n(cell detection error)", y = "Percentage", fill = "Object") +
...
plot <- ggplot(ueF_mucosal, aes(x = "", fill = factor(quality_issue))) +
  geom_bar(position = "fill") +
  labs(x = "Unequivocal FP & FN \n(mucosal model errors)", y = "Percentage") +
...
tiff(output_file_tif, width = 1.9, height = 2.33, units = "in", res = 300)
print(plot)
dev.off()
...
plot <- ggplot(ueFN_lc, aes(x = "", fill = factor(quality_issue))) +
  geom_bar(position = "fill") +
  labs(x = "Unequivocal FN \n(cell detection errors)", y = "Percentage", fill = "quality issue") +
 scale_fill_manual(values = quality_palette, name = "Quality", labels = quality_labels) +
  scale_y_continuous(labels = scales::percent_format(scale = 100)) +
  theme_bw() +
  theme(legend.position = "none", 
...
#Plot unequivocal FP with lc issues  by quality issue  
desired_order <- c("none", "stain", "focus", "tissue", "stain and focus", "stain and tissue", "tissue and focus", "stain, tissue and focus")
...
plot <- ggplot(ueFP_lc, aes(x = "", fill = factor(quality_issue))) +
  geom_bar(position = "fill") +
  labs(x = "Unequivocal FP\n(cell detection errors)", y = "Percentage") +
  scale_fill_manual(values = quality_palette, name = "Quality", labels = quality_labels) +
  scale_y_continuous(labels = scales::percent_format(scale = 100)) +
  theme_bw() +
  theme(legend.position = "none", 
